...
/* This file is copied from Apache Tomcat. - Thanks!
 * Changed only package name and some parts of cookie datetime handling. */
...
public class CookieTools {
    ...
    public static void getCookieHeaderValue(Cookie cookie, StringBuffer buf) {
        ...
        if (cookie.getMaxAge() >= 0) {
            if (version == 0) {
                buf.append ("; Expires=");
                synchronized(nscpCookieFormat) {
                    if (cookie.getMaxAge() == 0)
                        nscpCookieFormat.format(new Date(10000), buf,
                                                        new FieldPosition(0));
                    else
                        nscpCookieFormat.format
    ...
    private static DateFormat nscpCookieFormat=null;
    static {
        nscpCookieFormat = new SimpleDateFormat("EEE, dd-MMM-yyyy HH:mm:ss", Locale.US);
